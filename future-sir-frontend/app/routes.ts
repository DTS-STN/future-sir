...
import { index, layout, route as rrRoute } from '@react-router/dev/routes';
...
function i18nPageRoutes(i18nPageRoute: I18nPageRoute): RouteConfigEntry[] {
  return Object.entries(i18nPageRoute.paths).map(([language, path]) => {
    return rrRoute(path, i18nPageRoute.file, { id: path });
  });
}
...
export default [
  ...
  rrRoute('*', 'routes/not-found.tsx'),

  rrRoute('/api/readyz', 'routes/api/readyz.ts'),
