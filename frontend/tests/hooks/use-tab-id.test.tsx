...
describe('use-tab-id', () => {
  ...
  it('should generate a new tab id if none exists in session storage', () => {
    ...
    expect(globalThis.sessionStorage.getItem(SESSION_STORAGE_KEY)).toEqual(result.current.tabId);
  });

  it('should update the URL if it does not match existing tab id', () => {
    globalThis.sessionStorage.setItem(SESSION_STORAGE_KEY, 'xx-0000');
  ...
  it('should re-render when session storage changes', () => {
    ...
    act(() => {
      globalThis.sessionStorage.setItem(SESSION_STORAGE_KEY, 'xx-1111');
