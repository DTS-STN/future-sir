...
/**
 * Generates an internationalized (i18n) redirect response.  This function
 * constructs a redirect URL based on the provided route file, resource (used to
 * determine the language), and optional parameters and search parameters.  It
 * then creates a `Response` object with the appropriate redirect status code
 * (default 302 Found) and the "Location" header set to the generated URL.
 *
 * @param i18nRouteFile - The filename of the i18n route definition (e.g.,
 *   "product-details"). This file should correspond to an entry in your
 *   `i18nRoutes` configuration.
 * @param resource - The request, URL, or path from which to extract the
 *   language. This can be a `Request` object (e.g., from a server-side request),
 *   a `URL` object, or a URL string.  The language is determined based on the
 *   resource.
 ...
 *   This can include the status code (e.g., `301` for a permanent redirect) and
 *   other response headers. Defaults to `{}`.
 * @param opts.params - Optional route parameters.  These are used to populate
 *   dynamic segments in the route path (e.g., `/:id` in a route definition).
 *   Should be an object where keys are the parameter names and values are their
 *   corresponding values.
 * @param opts.search - Optional search parameters (query string parameters).
 *   Should be a `URLSearchParams` object.
